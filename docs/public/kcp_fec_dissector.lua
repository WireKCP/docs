-- create protocol
kcp_fec_protocol = Proto("KCP-FEC",  "KCP Protocol with FEC Feature")
...
kcp_fec_protocol.fields = {conv, cmd, frg, wnd, ts, sn, una, len, content}

-- dissect each udp packet
function kcp_fec_protocol.dissector(buffer, pinfo, tree)
  length = buffer:len()
  ...
  pinfo.cols.protocol = kcp_fec_protocol.name
  ...
  while offset < buffer:len() do
      ...
      local subtree = tree:add(kcp_fec_protocol, buffer(), tree_title)
...
kcp_fec_protocol:register_heuristic("udp", function(buffer, pinfo, tree)
    if buffer:len() >= 24 then
        local cmd_val = buffer(12, 1):le_int()
        if cmd_val == 81 or cmd_val == 82 or cmd_val == 83 or cmd_val == 84 then
            kcp_fec_protocol.dissector(buffer, pinfo, tree)
            return true
        end
    end
    return false
